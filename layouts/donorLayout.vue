<template>
    <nav class="flex flex-col sm:flex-row border-b border-slate-300 py-5 justify-between items-center">
        ...
        <ul class="flex items-center justify-center sm:justify-end gap-3 w-full sm:w-3/4 pt-5 sm:pt-0">
            <li v-if="!user"><NuxtLink to="/donor/login" class="hover:underline outline-offset-8">Login</NuxtLink></li> | 
            <li v-if="!user"><NuxtLink to="/donor/register" class="hover:underline outline-offset-8">Register</NuxtLink></li>
            <li>{{ user }}</li>
